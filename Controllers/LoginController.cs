...
{
    private static readonly string[] AllowedWorkspaceRoles = ["Support Agent"];
    private const string AllowedWorkspaceRolesLabel = "Support Agent";
    ...
    public IActionResult AdminLogin()
    {
        ...
        var startupToastMessage = TempData["LoginError"] as string;
        return Content(BuildLoginPageHtml(startupToastMessage), "text/html");
    }
    ...
    public async Task<IActionResult> AuthenticateAdmin([FromBody] LoginRequestModel request)
    {
        ...
        await LogAdminAction(
            ...
            "Success",
            $"Successful agent workspace login as {response.User.UserType}");
        ...
        try
        {
            ...
            if (await reader.ReadAsync())
            {
                if (reader["OTPCode"] != DBNull.Value)
                {
                    ...
                    var userName = reader["UserName"]?.ToString() ?? "Support Agent";
    ...
    public async Task<IActionResult> Logout()
    {
        ...
        if (staffId.HasValue)
        {
            await LogAdminAction(staffId.Value, "Logout", "Agent Workspace", "Success", "User logged out");
    ...
    private string BuildLoginPageHtml(string? startupToastMessage)
    {
        var viewPath = Path.Combine(_environment.ContentRootPath, "Views", "Login", "AdminLogin.cshtml");
        var html = System.IO.File.ReadAllText(viewPath);
        var documentStart = html.IndexOf("<!DOCTYPE html>", StringComparison.OrdinalIgnoreCase);
        html = documentStart >= 0 ? html[documentStart..] : html;
        html = html.Replace("@ViewData[\"Title\"]", "Agent Login", StringComparison.Ordinal);
        html = html.Replace("href=\"~/", "href=\"/", StringComparison.Ordinal);
        html = html.Replace("src=\"~/", "src=\"/", StringComparison.Ordinal);

        html = Regex.Replace(
            html,
            @"\s*@if\s*\(!string\.IsNullOrWhiteSpace\(startupToastMessage\)\)\s*\{[\s\S]*?\}\s*</body>",
            BuildStartupToastScript(startupToastMessage) + Environment.NewLine + "</body>",
            RegexOptions.Multiline);

        return html;
    }

    private static string BuildStartupToastScript(string? startupToastMessage)
    {
        if (string.IsNullOrWhiteSpace(startupToastMessage))
        {
            return string.Empty;
        }

        var encodedMessage = JsonSerializer.Serialize(startupToastMessage);
        return $$"""
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                showToast({{encodedMessage}}, false);
            });
        </script>
        """;
    }
