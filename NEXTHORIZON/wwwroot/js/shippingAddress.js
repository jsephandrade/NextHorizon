...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function readSeedAddress() {
        const seedElement = document.getElementById('shippingAddressSeed');
        if (!seedElement) {
            return null;
        }

        try {
            const parsed = JSON.parse(seedElement.textContent || 'null');
            return parsed && typeof parsed === 'object' ? parsed : null;
        } catch (e) {
            console.error("Error parsing seeded shipping address:", e);
            return null;
        }
    }
    ...
    function buildAddressCard(data) {
        ...
        const streetLine = (data.fullAddress || `${data.house || ''}${data.building ? ', ' + data.building : ''}${data.street ? `${data.house || data.building ? ', ' : ''}${data.street}` : ''}`).trim();
        const areaParts = [
            data.barangay,
            data.city,
            data.province,
        ...
        const contactParts = [data.fullName, data.phone].filter(value => value && value.trim());
        const areaLine = areaParts.length > 0
            ? areaParts.join(', ')
            : contactParts.join(' • ');
        ...
        newCard.innerHTML = `
            ...
            <div class="address-details">
                <p class="street-line"><strong>${streetLine || 'ADDRESS'}</strong></p>
                <p class="area-line">${areaLine || 'Saved shipping address'}</p>
            </div>
    ...
    saveBtn.addEventListener('click', function (e) {
        ...
        const newCard = buildAddressCard({
    ...
    addressList.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('.edit-btn')) {
            ...
            streetInput.value = card.dataset.street || card.dataset.fullAddress || '';
    ...
    // Initialize
    const seedAddress = readSeedAddress();
    if (seedAddress && seedAddress.address) {
        addressList.appendChild(buildAddressCard({
            fullAddress: seedAddress.address,
            fullName: seedAddress.fullName,
            phone: seedAddress.phone,
            isDefault: seedAddress.isDefault !== false
        }));
    }
