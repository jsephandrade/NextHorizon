    ...
    public class HomeController : Controller
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var orderItemImageColumn = FindColumn(orderItemsColumns, "ProductImage", "ProductImagePath", "ImagePath", "Image");
                var orderItemImageDataColumn = FindColumn(orderItemsColumns, "ProductImageData", "ProductImageBinary", "ImageData");
                var orderItemImageMimeColumn = FindColumn(orderItemsColumns, "ProductImageMimeType", "ProductImageMime", "ImageMimeType");
                ...
                foreach (var item in vm.CartItems)
                {
                    ...
                    // Persist the selected/normalized image path with the order item when the table supports it.
                    if (!string.IsNullOrWhiteSpace(item.Image) && !string.IsNullOrWhiteSpace(orderItemImageColumn))
                    {
                        AddOrderItemField(orderItemImageColumn, "@ProductImage", _mediaPathService.NormalizePublicPath(item.Image), DbType.String);
                    }

                    // If OrderItems supports binary image columns, attempt to capture the
                    // variant/product image bytes at the time of ordering so MyPurchases
                    // can serve the exact image the buyer ordered.
                    if (!string.IsNullOrWhiteSpace(orderItemImageDataColumn) || !string.IsNullOrWhiteSpace(orderItemImageMimeColumn))
                    {
                        await using var imgCmd = connection.CreateCommand();
                        imgCmd.Transaction = transaction;
                        var imgSql =
                            "SELECT TOP (1) pv.ImageData AS VariantImageData, pv.ImageMimeType AS VariantImageMimeType, p.ImageData AS ProductImageData, p.ImageMimeType AS ProductImageMimeType, pv.ImagePath AS VariantImagePath, p.ImagePath AS ProductImagePath FROM dbo.ProductVariants pv LEFT JOIN dbo.Products p ON p.ProductId = pv.ProductId WHERE pv.ProductId = @ProductId";

                        if (!string.IsNullOrWhiteSpace(item.Size))
                        {
                            imgSql += " AND (pv.Size IS NULL OR LTRIM(RTRIM(CONVERT(NVARCHAR(100), pv.Size))) = @Size)";
                            AddDbParameter(imgCmd, "@Size", item.Size, DbType.String);
                        }

                        imgCmd.CommandText = imgSql;
                        AddDbParameter(imgCmd, "@ProductId", item.ProductId, DbType.Int32);

                        await using var imgReader = await imgCmd.ExecuteReaderAsync(cancellationToken);
                        if (await imgReader.ReadAsync(cancellationToken))
                        {
                            // Prefer variant image data then product image data
                            var variantImageOrdinal = imgReader.GetOrdinal("VariantImageData");
                            var productImageOrdinal = imgReader.GetOrdinal("ProductImageData");
                            if (!imgReader.IsDBNull(variantImageOrdinal))
                            {
                                var bytes = (byte[])imgReader.GetValue(variantImageOrdinal);
                                AddOrderItemField(orderItemImageDataColumn, "@ProductImageData", bytes, DbType.Binary);
                                var mime = imgReader.IsDBNull(imgReader.GetOrdinal("VariantImageMimeType")) ? null : imgReader.GetString(imgReader.GetOrdinal("VariantImageMimeType"));
                                AddOrderItemField(orderItemImageMimeColumn, "@ProductImageMimeType", mime, DbType.String);
                            }
                            else if (!imgReader.IsDBNull(productImageOrdinal))
                            {
                                var bytes = (byte[])imgReader.GetValue(productImageOrdinal);
                                AddOrderItemField(orderItemImageDataColumn, "@ProductImageData", bytes, DbType.Binary);
                                var mime = imgReader.IsDBNull(imgReader.GetOrdinal("ProductImageMimeType")) ? null : imgReader.GetString(imgReader.GetOrdinal("ProductImageMimeType"));
                                AddOrderItemField(orderItemImageMimeColumn, "@ProductImageMimeType", mime, DbType.String);
                            }
                        }
                    }
