...
{
    public UpdateMemberUploadRequestValidator()
    {
        RuleFor(x => x.Title)
            .NotEmpty()
            .MinimumLength(3)
    ...
    private static bool HaveExactlyOneDistanceValue(UpdateMemberUploadRequest request)
        => request.DistanceKm.HasValue ^ request.DistanceMi.HasValue;

    private static bool HaveConsistentDistanceAndTime(UpdateMemberUploadRequest request)
    {
